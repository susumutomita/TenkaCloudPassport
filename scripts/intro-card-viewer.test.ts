...
/**
 * Issue 104 / ADR-0036: 会話テーマ ID（`m`）の payload allowlist・カタログ複製が
 * `src/protocol/intro-card-url.ts` / `src/domain/clue-catalog.ts` / `src/domain/intro-card.ts`
 * の正本とドリフトしていないことを固定する。
 */
describe('payload allowlist・会話テーマ ID（m）の表示契約', () => {
