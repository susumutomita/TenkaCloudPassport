...
/**
 * 実 jscpd が実 file を走査する最小 fixture repo を作る。src/lib/a.ts と src/lib/b.ts は
...
function makeFixtureRoot(): string {
  ...
  writeFileSync(path.join(root, 'src', 'lib', 'a.ts'), body);
  writeFileSync(path.join(root, 'src', 'lib', 'b.ts'), body);
...
describe('areaOf', () => {
  it('src 配下は directory 単位の area を返す', () => {
    expect(areaOf('src/domain/passport.ts')).toBe('src/domain');
    expect(areaOf('src/screens/backup.tsx')).toBe('src/screens');
    expect(areaOf('src/local-agent/runtime.ts')).toBe('src/local-agent');
  });

  it('それ以外は最上位 entry を area として返す', () => {
    expect(areaOf('scripts/check-duplication.ts')).toBe('scripts');
    expect(areaOf('App.tsx')).toBe('App.tsx');
...
describe('aggregateDuplicatedLines', () => {
  it('クローンの行数を両端の area にそれぞれ加算する', () => {
    const totals = aggregateDuplicatedLines([
      clone('src/domain/a.ts', 'src/protocol/b.ts', 10),
      clone('src/domain/c.ts', 'src/domain/d.ts', 7),
    ]);
    expect(totals).toEqual({ 'src/domain': 24, 'src/protocol': 10 });
  });
});
...
describe('runJscpd', () => {
  it('実 jscpd を実行して report JSON を返す', () => {
    ...
    expect(areas).toEqual(['scripts', 'src/lib']);
  });
...
describe('main', () => {
  it('--update は baseline を現状の重複量で書き出して 0 を返す', () => {
    ...
    expect(baseline['src/lib']).toBeGreaterThan(0);
  });
  ...
  it('重複が baseline を超えたときは 1 を返す', () => {
    const root = makeFixtureRoot();
    main(['--update'], root);
    const body = readFileSync(path.join(root, 'src', 'lib', 'a.ts'), 'utf8');
    writeFileSync(path.join(root, 'src', 'lib', 'c.ts'), body);
    expect(main([], root)).toBe(1);
  });
