...
architecture harness は、TenkaCloudPassport の repository state に対して決定論的に判定できる契約だけを機械強制します。思考手順、開発順序、role play は対象外です。

## Enforced categories

- dependency lifecycle、lockfile、GitHub Actions、既知 IOC などの supply-chain boundary。
- focused test、型エスケープ、未実装 marker、production source の mock data などの客観的シグナル。
...
- mobile app の auth storage、unsafe HTML、source root、build contract など repository 固有の invariant。
...
CI は全件 harness、typecheck、test、coverage、build / export を最終強制点として実行します。
