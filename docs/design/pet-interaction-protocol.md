...
**追記（Issue 11）**: Issue 11 は、本書が Known follow-ups に残していた 2 件（Owner
Question の入力 UX、bounded protocol の実判定経路への接続）を実装した。詳細は
[Owner Question の段階的開示・Consent Flow の設計](./owner-question-consent-flow.md)
を参照する。`PassportApp.tsx` は `src/app/pet-interaction-flow.ts` を介してこの
bounded protocol を Active Lounge の実判定へ配線し、Rules Provider による直接の
`evaluateLounge` 呼び出しはアプリの実行経路から外した。ただし `evaluateLounge` /
`RULES_PROVIDER` 自体は削除せず、100％ カバレッジ済みの既存 API として
`src/domain/lounge.ts` / `src/domain/rules-provider.ts` にそのまま残る（Known
follow-ups 参照）。
...
- 実際の Local Agent（`llama.rn`）を `InteractionDiscoveryProvider`（`kind: 'local-agent'`）。
  として実装し、`createLazyLocalAgent` と同じ遅延 loader 境界に接続する。この接続が完了する
  まで、Discovery Provider は `RULES_INTERACTION_PROVIDER` の同期判定のままである。
- Issue 11 で解消済み: この bounded protocol を Active Lounge の実判定経路へ接続する配線
 （`src/app/pet-interaction-flow.ts`）と、`clarifying` の Owner Question を実際に Owner へ
  提示する UI（`src/screens/OwnerQuestionScreen.tsx`）。詳細は
