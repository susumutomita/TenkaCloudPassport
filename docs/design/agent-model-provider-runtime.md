...
一方、Active Lounge 中の Model Activate / Unload / Delete は Lounge の終端ではない。遅延結果の適用 Gate と
Runner の実行・確定 Ledger は取り消すが Encounter Key は保持し、teardown 後の Rules または再構成済み Local
Provider を同じ Lounge から再開できるようにする。
結果適用 Gate は Encounter Key だけでなく単調増加する世代 Token を払い出す。旧 teardown 中に同じ Key の新しい
実行が始まっても、旧 Handler は新世代 Token を settle できない。Model mutation は UI が保持する現在 Key ではなく
Runner の全 pending Native teardown を `cancelAllAndWait()` で drain するため、Lounge 終端で Key を Forget 済みでも
Context release 前に File を変更しない。同じ Key の旧・新 teardown は別 record として保持し、旧完了が新 record を
消さない。release quarantine は未解決 Promise で後続を永久待機させず、後続 Lane にも内容非保持の型付き
`LOAD_ERROR` として伝播し、Process 再起動まで Native Provider と Model mutation を拒否する。
Timeout / fallback の Outcome は Native teardown より先に確定できるため、Rules fallback、期限判定、Pilot Outcome は
release を待たず直ちに適用する。同時に Ledger を破棄しない `waitForNativeTeardowns()` を独立して開始し、UI の Provider
pending はこの drain 完了後だけ解除する。待機中は Settings の自動 reload、Model 候補選択、次の Provider 開始を拒否する。
Native が Abort を無視して release が未完了でも、確定済み Outcome の表示を永久に待たせない。release quarantine では pending を
解除せず再起動境界を保つ。drain 中に Owner が Model mutation を確定した場合だけ、drain 後の `cancelAllAndWait()` で
確定 Ledger を破棄し、同じ Lounge から再構成 Provider を開始できる契約へ移る。
Native drain 完了は Model mutation の完了ではない。UI の mutation lane は Manifest 更新、staged File の最終処理、
Provider 再構成と refresh が終わるまで同期的な pending 判定を保持し、その間の Lounge Provider 開始と Settings 離脱を
拒否する。
