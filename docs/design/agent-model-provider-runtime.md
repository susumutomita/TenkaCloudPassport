...
再表示しない。異なる Encounter Key が並行完了しても、それぞれの Outcome を最新 Ledger へ
merge し、片方の結果で他方を失わない。

Local Provider は信頼境界なので、自発的に `TIMEOUT` を返すことを前提にしない。Runner は
`deadlineAtWallClockMs - Date.now()` を上限とする Timer と Provider 実行を競争させ、期限到達を
内容のない `timeout` Failure へ変換して Rules へ Fallback する。Timer は完了時に必ず解除する。
期限後に Local Provider が完了しても、その Promise は State や Ledger を更新する権限を持たず、
確定済み Rules Outcome を上書きしない。
...
Issue 16 では Expo Go / Web の Model 未導入状態が `rules` と表示され、既存 Rules フローを
妨げないことを確認する。`loading-local-model` と `local-model` の Development Build 実表示、
Native Cancel、実機 Airplane Mode は `llama.rn` Adapter を接続する Issue 17 の証跡対象とする。
