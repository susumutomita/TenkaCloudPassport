...
| Port | `AgentModelProvider` | `{ kind: 'rules' \| 'local-agent'; provide(input, { signal? }): unknown \| Promise<unknown> }`。Native 境界の実値は型注釈を信用せず共通 Validator へ渡す。Local 実装は Abort 後に停止・Resource 解放を終えてから Promise を settle する。 |
...
- `createAgentProviderSessionRunner()`: 実行中 Promise と確定済み Ledger を同じ Closure で所有し、
  ...
  完了しなければ Runner 自身が Signal を Abort して `timeout` の Rules Outcome を期限で確定する。停止・
  Resource 解放は別の Native Lane 所有権として追跡し、期限後の遅延成功は Outcome を上書きしない。Local
  実行は単一 Native Lane へ直列化し、前 Context の解放前に次 Context を開始しない。
...
Issue 17 で `llama.rn` Adapter をこの Contract へ接続した。Model 未設定、Load 失敗、構造化成功、
入力外 Evidence、Streaming Cancel、Context Release は統合 Test で検証する。Timeout の実測値、
Schema Error の実際の頻度、OOM と Model 不正の端末別挙動は、同 Issue の iPhone / Android arm64
実機 Matrix で人間検証する。

## Known follow-ups

- Issue 17 で `llama.rn` を canonical request だけを受ける `LocalModelCompletionPort` として実装し、
  `model-safety-boundary.ts` の唯一の Local capability factory を経て `createAgentProviderSessionRunner()` と
  `pet-interaction-flow.ts` の Live 経路へ接続した。Model の
  `no-signal` は既存 `RULES_INTERACTION_PROVIDER` / Owner Question へ戻し、検証済み Bridge は
  `agent-model-live-outcome.ts` が既存 Bridge constructor だけで具体化する。
- `loading-local-model` / `local-model` の Development Build 実表示、実機 OOM、Airplane Mode 成功は
  Issue 17 の iPhone / Android arm64 実機 Matrix に残る。
- 旧 `evaluateLounge` / `RULES_PROVIDER` を削除するか、運用ツール用 API として残すかは M3 の
  Wire 配線時に判断する。Issue 17 の Live 経路では `AgentModelProvider` を単一の Model 境界とする。
