...
- System Instruction と、信頼側 Domain が同意済み Public Passport から導出した Evidence-only JSON Data を
  別 Message にする。表示文字列は Model へ渡さない。
...
PassportApp（Encounter Key と Lounge lifetime を所有）
  -> createAgentProviderSessionRunner（deadline / cancel / fallback-once / idempotency）
    -> llama AgentModelProvider（Development Build のみ）
      ...
      -> completion
         system message: 固定命令
         user message: 許可済み Evidence だけの版付き JSON
         response_format: strict JSON Schema
      -> 4 KiB・深度 4 の bounded JSON parse（外部入力境界）
...
| `agent-inference-safety.ts` | Local Agent Input の strict schema、危険 Unicode、Evidence-only Prompt、Prompt / Output 上限を所有する。 |
| `llama-agent-model-provider.ts` | Native API の最小 Port、JSON Schema、Error 分類、Stop / Release を所有する。React と Expo を import しない。 |
...
重複なしにする。Grammar 制約だけは信頼せず、Completion Text を UTF-8 4 KiB・深度 4 の bounded JSON として
parse した後に Issue 16 の共通 Validator へ渡す。Model が入力外 Evidence、URL、Contact、Tool、自由記述 Claim を追加した場合は
...
System Message は固定文字列であり、Passport の値を連結しない。Local Agent 実行直前に Input を strict
schema で再検証し、危険 Unicode を拒否する。User Message は `{ schemaVersion, allowedEvidence }` の
4 KiB 以下の JSON だけとし、Public Passport の `petName`、`ownerAlias`、Owner Answer 本文を含めない。
Native Log、Error Message、UI State に Passport、Prompt、Model Output を保存しない。詳細は
[Local Agent の入力・出力安全境界](./local-agent-input-output-safety.md) と
[ADR-0013](../adr/0013-minimize-local-agent-prompt-data.md) を正本とする。
...
| Local Agent Input の strict schema / Unicode、Prompt 4 KiB、Output 4 KiB・深度 4 が不正 | `SCHEMA_ERROR` | Model 初期化前または Runtime Validation 前に全体を破棄し Rules へ 1 回切替。 |
...
- Pet 名、Alias に Plain Text の命令文、URL、Contact、端末 Path があっても Prompt へ含めない。Unicode
  双方向制御、Zero-width、制御文字は Model 初期化前に拒否する。
- Native が生成 Token 上限を無視しても、Output の UTF-8 4 KiB・深度 4 を超えて JSON parse と共通
  Validator へ進めない。
...
Evidence、Prompt Injection Corpus、1,000 件以上の Fuzz Input、Prompt / Output 上限、Timeout、Streaming
Cancel、Release、二重実行、Web Export を日本語 BDD Test で固定する。
