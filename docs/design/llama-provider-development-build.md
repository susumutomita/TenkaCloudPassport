...
### memory entitlement（`extended-virtual-addressing` / `increased-memory-limit`）の preview 限定復元

Issue 104 / ADR-0037（Bonsai-ready 化）で `app.json` の `llama.rn` plugin
`entitlementsProfile` を `["development"]` から `["preview", "production"]` へ
一度戻したが（follow-up `01KY4W8FE17WMR049WB3VW8T2M`）、PR #132（Issue 104
Codex レビュー）で `["preview"]` 限定へ修正した。**production を含めない。**

過去の実障害（PR #116、commit `cf08728e`）は「Beta App Review で問題になった」
という推測ではなく、**TestFlight ビルドの署名段階そのものが失敗した**という
事実を指す。EAS のビルドログには次のエラーが出た。

```text
Provisioning profile does not include the com.apple.developer.kernel.extended-virtual-addressing
and com.apple.developer.kernel.increased-memory-limit entitlements
```

これは Apple Developer Portal で対象 App ID の capability を有効化し、
Provisioning Profile を再生成しない限り、署名済み IPA 自体を作れない
（審査に到達する前の段階で止まる）ことを意味する。PR #116 はこれを受けて
production を development 限定へ縮小し、Issue 104 の Bonsai-ready 化（PR
#131）が preview・production 双方へ一旦復元したが、v1.0 で実際に採用する
Qwen2.5-1.5B-Instruct（Q4_K_M、約 1.06 GiB）はこの entitlement を前提にせず
Resource Risk `supported` に収まる設計目標のため、production へ復元する
合理性が無い。

**preview であっても、Apple Developer Portal の対象 App ID
（`cloud.tenka.passport`）で Increased Memory Limit capability を有効化し、
Provisioning Profile を再生成しない限り、EAS の preview ビルドも同じ署名
エラーで失敗する。** これは
本 Repo のコード・CI では検証できない owner 実施ゲートであり、`app.json` の
設定変更だけでは有効化されない。owner が実施する前提条件と検証手順は次のとおり。
...
2. EAS Build で preview profile の署名済み IPA を作成し、Provisioning
   Profile に entitlement が反映されていることを `codesign -d --entitlements`
   等で確認する（production はこの手順の対象外のまま維持する）。
3. 実機（iPhone 15 Pro 以降を推奨）の preview ビルドで、上記の物理端末実機証跡
  （`loading-local-model` → `local-model` → Bridge 完走）を development build
   のときと同じ手順で取り直す。
4. Bonsai 1-bit 等の大型 Model を将来 production へ出荷する場合だけ、
   `entitlementsProfile` へ `production` を追加する別 PR を起票し、上記と同じ
   capability 有効化・Provisioning Profile 再生成・TestFlight 提出・実機証跡を
   production 向けに取り直す。
