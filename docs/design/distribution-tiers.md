...
Android の Release metadata は APK 内に埋め込んだ Source Tag / Commit、実 Manifest の Package ID / versionCode、
実 Signer Certificate SHA-256、APK SHA-256 を同じ strict manifest へ結合した場合だけ有効である。外部の
`app.json`、Tag 名、checksum を個別に目視した結果から Binary identity を推測しない。
Identity Gate は checksum 済み APK の private read-only snapshot を Android SDK Tool 全 command で共有し、
Tool 実行後に snapshot と元 APK / checksum pair を再検証する。設定は検証済み Tag / HEAD の `app.json` blob、
実行系は replace / 継承環境を無効化した Git と、承認済み SDK / Java dependency tree の SHA-256 に固定する。
macOS の system Git shim は拒否して canonical な実 Git executable を直接固定する。SDK / Java tree は byte / entry / depth
を読取前から制限し、private read-only snapshot へ固定する。`apkanalyzer` classpath と `apksigner.jar` は snapshot の
承認済み Java から直接実行する。Tool timeout、
fingerprint 不一致、APK 512 MiB 超過、検証後の manifest 不一致では metadata を残さない。
