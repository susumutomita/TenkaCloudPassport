...
```text
TCPQ1:{"qrProtocolVersion":{"major":1,"minor":1},"kind":"lounge-invite","value":{...}}
```

`kind` が `lounge-invite` の場合、`value` は Host が `src/domain/lounge-invite.ts` の
`createLoungeInvite` で作った `{ schemaVersion, loungeId, expiresAtEpochMs }` だけを持つ。
...
満員は Room の現在状態に依存するため、domain 層の `joinLoungeRoom` に置く。この分離により、
QR の内容そのものの妥当性と、参加時点の Room の状態を別の関心事として検証できる。
