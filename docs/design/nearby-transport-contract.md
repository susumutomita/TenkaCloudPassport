...
- Payload、raw Join Request UTF-8、Queue、Send Rate、rolling byte、Peer、Listener を有限にする。
...
### 案 1: Peer Protocol セッションに Network 接続と Queue を持たせる
...
Expo Go / Web の Rules Flow へ Native import が混入し、同じ Contract Test を実 Adapter へ適用できない。
...
認証 callback、現在 Membership、bounded Queue だけとする。
...
Secure セッション Resume / 再認証方式は Issue 20 の ADR に従う。
...
| raw Join Request UTF-8 | 1,024 byte | `INVALID_CONFIGURATION` |
...
実 Native Adapter の完了には以下が別途必要となる。

- Issue 20 の Accepted Transport ADR と Production Library。
- Loopback と実 Adapter の同一 Contract Suite Green。
