...
[保持ポリシー](../privacy/retention-policy.md)、初回設定は
[Passport 初回設定](./passport-onboarding.md) を正本とする。
...
1. Owner は Pet Name、Pet Emoji、任意 Alias、版管理済みの手掛かりと Languages を入力し、
   Local Private Profile を明示保存する。氏名、連絡先、位置情報、URL、機密情報は入力しない。
2. 同じ端末で、相手がその場で公開した Pet Name と 1 件以上 3 件以下のカタログ項目を入力し、
   公開確認を行う。
3. Owner は Lounge 参加直前の Preview で Pet Name、任意表示項目、各手掛かり、各 Language を
   項目単位で ON / OFF にする。Pet Name と手掛かり 1 件以上を有効条件とする。
4. アプリは Lounge の開始操作で Preview と同じ 2 つの Public Passport を投影し、メモリだけに保持する。開始時に
...
| --- | --- | --- | --- |
| Local Profile 編集中 | 未保存 Draft である。 | 入力、明示保存である。 | Encounter 準備である。 |
| Encounter 準備 | 明示保存済み Local Profile と相手の公開入力である。 | 相手の公開確認、Local Profile 編集である。 | 共有 Preview または Local Profile 編集中である。 |
| 共有 Preview | 今回の項目別 ON / OFF である。 | Lounge 開始、取消である。 | `active` または Encounter 準備である。 |
...
- Passport は表示名とカタログ値の上限、重複拒否、明示確認、最大 3 件の手掛かりを含む公開投影を担当する。
...
- アプリの再読み込み後は以前の Lounge と未保存 Draft を再水和せず、明示保存済み Local Profile だけを
  復元して新しい Encounter から始める。
