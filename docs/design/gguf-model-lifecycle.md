...
| `agent-provider-session.ts` | 実行中 Context を Abort し、Native teardown 完了を待てる `cancelAndWait` を所有する。 |
...
4. Candidate の Name / Size だけを画面に出し、Owner の Import 確定を待つ。URI は表示・ログ保存しない。
   確定後も中止操作を表示し、Abort を Copy 後、Hash 中、Metadata 確認後、最終 rename 後の各境界で再評価する。
5. `.incoming.gguf` へ Copy し、Copy 後 Size が選択時 Size と一致しなければ File を削除して
   `COPY_INCOMPLETE` にする。
6. 1 MiB chunk で SHA-256 を計算する。Read permission 失効は `SOURCE_UNREADABLE`、途中中断は
...
- 実行中 Provider に影響する Activate / Unload / active Delete は、現在の判定を終了する明示確認なしでは実行しない。
