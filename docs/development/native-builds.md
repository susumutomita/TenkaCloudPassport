...
Delivery の判断は [ADR-0010](../adr/0010-native-delivery-quality-gates.md)、Pilot の配布 Tier は
[Pilot 配布 Tier と Scale Gate](../design/distribution-tiers.md) を正本とする。
Native Binary の Runtime だけから直接配布、Personal Team、TestFlight、Store を推測せず、App 内の Tier は
署名済み Release metadata がない限り未判定にする。
...
| iOS Development Build | 利用できる。 | Files から GGUF を取り込むか、開発者用 Path を設定した Build で利用できる。 | iOS code signing、権限、Native link、実機 runtime の動作である。 |
| Android Development Build | 利用できる。 | Files から GGUF を取り込むか、開発者用 Path を設定した Build で利用できる。 | Android SDK、権限、ABI、Native link、実機 runtime の動作である。 |
...
Issue 18 の Model Management 画面から取り込んだ managed Model がある場合は、private Manifest を起動時に
再検証して環境変数より優先する。Document Picker は Size 表示前の cache copy を行わず、Owner 確定後だけ
private models directory へ Copy する。`supported` は activate でき、`caution` は明示確認が必要で、`blocked`
...
bunx expo-modules-autolinking resolve --platform android --json
```

手順と Compatibility Matrix は
[GGUF Import・Resource Guard・Benchmark の設計](../design/gguf-model-lifecycle.md) を参照する。
...
Personal Team では同時に登録できる App ID は 10 件、test device は 3 台、1 device に
install できる app は 3 件です。App ID、device、Provisioning Profile は 7 日で失効し、期限後は Build と
install をやり直します。これは個人所有端末の検証境界であり、複数人への継続配布には使いません。
...
- private Model Manifest と Local Benchmark Report も commit しない。Report を確認するときも Passport、Prompt、
  Answer、Bridge、File URI、端末 ID が含まれないことを先に検査する。
