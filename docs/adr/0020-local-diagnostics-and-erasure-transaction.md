...
fresh process の Model lease registry は recovery-locked で開始し、marker 確認前の Context 取得と Model mutation を許さない。
起動回復の失敗時は通常 UI を解放せず、Diagnostic Recovery 専用状態の明示再試行が成功した後だけ lock を解除する。
