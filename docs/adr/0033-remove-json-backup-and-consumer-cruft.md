...
- **配布能力デバッグ表示を Settings から削除し、orphan になったモジュール群も
  同じ PR で削除して掃除を完遂する**: `distributionCapability` prop を
  ...
  を外す。この結果 `DistributionCapability` 型（`distribution-capability.ts`）・
  `distribution-capability-notice.ts`・`default-distribution-capability*.ts`
  （`.ts` / `.native.ts` / `.web.ts`）は生産コードからの唯一の呼び出し元を失う。
  当初は削除判断を follow-up 送りにしていたが、Codex レビューで
  「production 消費者が無いまま残すのは DoD（`docs/architecture/quality-bar.md`）の
  no-dead-code 違反」と指摘され、production 消費者不在を grep で再確認した上で
  同じ PR で削除した。専用テスト `distribution-capability.test.ts` も合わせて削除し、
  `default-agent-model-provider.test.ts` の test-only 参照（Issue 28 の
  Runtime capability source-text 検査）も削除に追従させた。App Composition の
  AgentModelProvider/LocalModel 契約を検証するテストはそのまま残す。未使用になった
  `settings.distribution` 系 i18n メッセージ（`app/i18n/messages.ts` の型・`ja`・`en`
  各ブロック）も同じ PR で削除した。
...
- **Tradeoff**: `BackupSharePort` 等の命名は「Backup」を冠したまま残るため、
  ...
  関連モジュールは、上記の通り production 消費者不在を確認した上で本 PR 内で
  削除済みであり、再判定を先送りする tradeoff は残らない。
