...
5. **Apple の秘密情報（appleId / API キー / provisioning / Team ID）はワークフローにも
   ...
   完結する。ただし `submit.production.ios.ascAppId`（App Store Connect 上で公開される
   数値 App ID）は例外として扱う。`eas submit --non-interactive` は対象アプリを一意に
   特定できないと失敗するため、この値だけはプレースホルダとして `eas.json` に committed し、
   owner が App Store Connect でアプリ枠を作成した後に実際の数値へ書き換える
   （非秘密情報であり、コードレビューで指摘され追加した）。
