...
| Local Private Profile | `L1` | Owner がカタログから選択する。 | `schemaVersion`、手掛かり候補、公開選択、除外トピックである。 | OS のアプリ専用保護領域である。 | Owner と端末内 Pet だけである。 | Owner が削除するまでである。 | 個別削除、Profile 初期化、アプリ削除である。 | 可である。 |
| Public Passport | `L2` | Owner の確認操作により Local Private Profile から投影する。 | `schemaVersion` と最大 3 件の公開許可済み手掛かりである。 | 現在の QR を作る間のメモリだけである。 | QR を見る人と参加を許可された Pet である。 | 投影から最大 20 分である。 | QR の閉鎖、再生成、退出、Host 終了、20 分満了のうち最も早い時点である。 | 否である。 |
...
| 位置情報、連絡先、URL、自由記述 | ない。 | ない。 | schema と UI の両方で入力経路を設けない。 |
...
1. Owner は版管理済みカタログから Local Private Profile の候補を選ぶ。
