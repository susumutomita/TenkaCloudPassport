...
データ項目と Export 可否は [Privacy データ台帳](./data-inventory.md) に従う。**Issue 118 /
ADR-0033 により手動 JSON バックアップ機能を削除したため、以下の表・本文中の
「バックアップ」列・記述は現在エクスポート経路が存在しないことを示す歴史的記録であり、
実際に復元・再エクスポートできることを意味しない。** Sanitized Diagnostic Report・
Pilot Event Aggregate の明示 Share は独立した経路として引き続き有効である。

## 基本原則

- `L3` の Lounge 由来データは永続ストレージ、ファイル、ログ、クラッシュレポートへ書き込まない。
- `L3P` の Process 限定集計は Process Memory だけに置き、Event Log、再水和処理、永続 Storage を設けない。
...
- Pilot 計測は個別 Event を保存せず、Process 内の固定 Counter だけを持つ。正確な時刻は duration Bucket 化の
  直後に捨て、Aggregate は Profile、バックアップ、Diagnostic、Lounge の永続状態へ混ぜない。
- Nearby Transport の raw Packet Capture は承認済み実機 Spike だけで取得し、暗号化した検証端末に置く。
  実在 Owner の Passport を使わず、Review 完了直後または取得から 7 日以内の早い方で削除する。
- raw `L5` から公開 Repository へ残せるのは、Privacy データ台帳の allowlist を満たす `L5P` Record への
  明示投影だけである。`L5P` は raw Capture の保持期限を延長せず、raw Capture の復元にも使わない。
- Formative Event Temporary Coded Record (`L6`) は Researcher だけが扱える暗号化済み一時領域に置き、
  Public Aggregate 更新直後またはセッション終了から 7 日以内の早い方で Record 全体を削除する。閉じる確認前の
  撤回と禁止 Field の発見は期限を待たず即時削除する。
- 公開 Repository へ残せるのは Privacy データ台帳の小 Cell 抑制を満たす `L6P` Public Aggregate だけである。
  `L6P` は元の `L6`、正確な人数、Role cohort、Locale cohort、Role × Locale、参加者の回答や会話に由来する
  自由記述、逐語引用を保持または復元しない。固定 Code から作る sanitized pattern summary は 140 文字以内とする。

## Owner による全削除

全削除の確認後は内容を持たない tombstone を最初に永続化し、この時点で Profile、Settings、バックアップ
...
| --- | --- | --- | --- | --- |
| Local Private Profile | Owner が明示保存してから削除するまでである。Draft は保存しない。 | 個別削除、Profile 初期化、アプリ削除である。 | 明示保存済みだけ復元できる。 | 明示 Export の対象である。 |
...
| Local Model Benchmark | Model ごとに直近 20 件までである。 | Model 削除、計測記録削除、設定初期化、アプリ削除である。 | private Manifest の strict 検証後だけ復元できる。 | 対象外である。 |
| Pilot Event Aggregate | Process 終了までである。未完了セッションと正確な duration は Aggregate へ残さない。 | Process 終了または全 Local Data 削除のうち早い時点である。 | 復元しない。 | Outcome 5 件以上を Preview 後に手動 Share した固定 JSON だけが対象である。 |
| Formative Event Temporary Coded Record | Public Aggregate 更新直後またはセッション終了から 7 日以内の早い方である。 | セッションを閉じる確認前の撤回、Aggregate 更新、7 日満了、禁止 Field の発見のうち最も早い時点で Record 全体を削除する。 | 復元しない。 | 対象外である。Privacy データ台帳の `L6P` allowlist への明示投影だけを許可する。 |
| Sanitized Formative Event Public Aggregate | Repository history と同じである。 | 誤りまたは禁止 Field は Public Aggregate と Review PR を Supersede して訂正する。 | 公開 Repository から再取得できる。元の `L6` は復元できない。 | 対象外である。固定 Code から作る 140 文字以内の sanitized pattern summary は許可するが、正確な人数、Role cohort、Locale cohort、Role × Locale、個別 Record、参加者由来の自由記述、逐語引用を含めない。 |
| Nearby Transport Spike raw Packet Capture | Review 完了直後または取得から 7 日以内の早い方である。 | Review 完了、7 日満了、手順逸脱の発見のうち最も早い時点である。 | 復元しない。 | 対象外である。Privacy データ台帳の `L5P` allowlist への明示投影だけを許可する。 |
| Sanitized Nearby Transport Spike Evidence Record | Repository history と同じである。 | 誤りは Record と Review PR を Supersede して訂正する。 | 公開 Repository から再取得できる。 | 対象外である。raw `L5`、端末 / Network 識別子、正確な日時を再構成できる Field は含めない。 |
...
- Protocol Message ID、sequence、Transport nonce、受理済み Replay 集合、送受信キューである。
...
- Pilot の個別 Event、未回答状態、正確な時刻、Ready の monotonic 値、未 Export Aggregate である。
- Nearby Transport Spike の raw Packet Capture、通信層 Address、正確な Packet Timing である。
...
## バックアップの保持境界（歴史的記録、Issue 118 / ADR-0033 で機能削除）

以前は Export が Owner の明示操作、対象の確認、OS の保存先選択を順に要求していた。JSON が
平文であり、選択した保存先の同期、共有、版管理、削除の挙動はアプリの管理外になることを
保存前に表示していた。手動 JSON バックアップ機能自体は削除済みであり、現在この経路は
存在しない。

Pilot Event Aggregate の Export も Owner / Facilitator の明示操作、全 field の Preview、OS の保存先選択を
順に要求する。Outcome 5 件未満では Preview JSON を生成しない。共有済みファイルはアプリから追跡、再送、
削除できず、Owner が保存先で管理する。アプリは Aggregate の生成時刻、保存先、共有先、共有履歴を保持せず、
Process 終了後に Counter を復元しない。

（以前の手動 JSON バックアップの Import は strict schema で未知フィールドを拒否し、Lounge
由来フィールドを無視して続行しなかった。検証に失敗したバックアップは取り込まず、既存の
Local Private Profile と端末設定を変更しなかった。この Import 機能自体は Issue 118 /
ADR-0033 で削除済みである。）
