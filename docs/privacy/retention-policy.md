...
- Pilot 計測は個別 Event を保存せず、Process 内の固定 Counter だけを持つ。正確な時刻は duration Bucket 化の
  直後に捨て、Aggregate は Profile、Backup、Diagnostic、Lounge の永続状態へ混ぜない。
...
| Pilot Event Aggregate | Process 終了までである。未完了 Session と正確な duration は Aggregate へ残さない。 | Process 終了または全 Local Data 削除のうち早い時点である。 | 復元しない。 | Outcome 5 件以上を Preview 後に手動 Share した固定 JSON だけが対象である。 |
