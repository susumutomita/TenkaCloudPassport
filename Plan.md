...
### [PR #127 Codex レビュー指摘の完遂] - 2026-07-23

#### 目的

`chore/remove-backup-and-consumer-cruft` ブランチ（PR #127、Issue 118）に対する Codex の
レビュー指摘（blocker 1 件・major 3 件・minor 2 件）をすべて解消し、同ブランチへ push する
（マージはしない。CI 緑と `release_verify` の確認は呼び出し元が行う）。

#### 制約

- 削除境界（`BackupSharePort`・`passport-share.ts` の温存）と「アサーション非弱化」は Codex が
  pass 済みのため維持する。
- `git rm` で削除する（`rm` コマンド禁止）。`npx` 禁止（`bunx` / `nlx`）。git add は明示ファイルのみ。
- 8081 kill 禁止、`ios/` / `node_modules` / `site/c/`（ビューア）は不触。

#### タスク

1. `scripts/source-release-inventory.json` から削除済み 19 パスを除去する（blocker 1）。
2. `site/index.html` / `site/en/index.html` の JSON Backup 記述を現挙動へ修正し、CSS specificity
   のみの変更を撤回する（major 2 / minor 6）。回帰テストを 1 本追加する。
3. Orphan な DistributionCapability モジュール群を production 消費者不在を再確認した上で削除し、
   未使用 i18n メッセージも除去、ADR-0033 を更新する（major 3）。
4. 診断まわりの `backup-import` / `backupCacheCount` 残骸を型・parse・シリアライズ・UI から除去し、
   Schema version を 1 → 2 へ明示的に上げる（major 4）。
5. `AppScreen.tsx` の新ヘッダーを font-scaling 契約の検査対象に加える（minor 5）。
6. `make before-commit` / `make release_candidate` / `make release_verify` / `make dead_code` を
   確認する。

#### 検証手順

- `bun test`（100% coverage 維持）、`bun run typecheck`。
- `make before-commit` が exit 0 になる。
- `make release_candidate` と `make release_verify`。
- `make dead_code` で新規 dead code が出ない。

#### 進捗ログ

- 2026-07-23: タスクファイルを読み、Codex の verbatim 指摘を把握した。隔離 worktree で
  `origin/main`（195ca54）と `origin/chore/remove-backup-and-consumer-cruft`（7bcb4bc）を fetch し、
  main が枝分かれしていない（7bcb4bc の親がそのまま 195ca54）ことを確認したため merge は不要と判断した。
- 2026-07-23: blocker 1 を修正する前に `make release_candidate` を実際に実行し、
  Codex が挙げた 19 パスとは**別の**、`.eas/build/ios-device-build.yml` で
  `FORBIDDEN_RELEASE_PATH` が先に throw されることを発見した。`origin/main`（195ca54、
  7bcb4bc より前）でも同じコマンドが同じ理由で失敗することを確認し、これは PR #127 の
  Backup 削除とは無関係な既存の破損（`.eas` / `modules` / `site` / `eas.json` / `wrangler.toml`
  という後発 PR で追加された root が `source-release.ts` の allowlist・reviewed inventory に
  一度も反映されていない）と判断した。さらに `site/og.png` が `assets/` 外の binary であり、
  `validateReleaseBlob` の `binaryAssets` は `assets/` prefix 必須という構造的制約もあるため、
  完全解消には「公開 OSS Source Release に何を含めるか」という ADR 級の設計判断が要ることを
  確認した。加えて、19 パスを除去した後の inventory と現在の Git tree を突き合わせたところ、
  無関係に見えて実は同じ根っこの問題として、reviewed inventory が現在追跡されている 165
  ファイルを網羅できていない（`origin/main` 時点で既に 164 ファイル分の同種の gap があった）
  ことも判明した。これは「reviewed」を謳う supply chain 成果物の性質上、1 ファイルずつ人手で
  追加していくべきものであり、cruft 削除 PR に黙って混ぜ込むべきではないと判断し、
  `/follow-up add` で 1 件（severity: high）に記録した上で、Codex が実際に指摘した
  「19 の stale パス除去」だけを本 PR の scope として実施した（除去後、現在の tree との
  stale diff が 0 件であることを python で確認済み）。
- 2026-07-23: `site/index.html` / `site/en/index.html` の「持ち出しは JSON Backup のみ」を
  「書き出しや自動バックアップの機能は持ちません」（EN: "There is no export or backup
  feature."）へ書き換え、`ol.flow` → `.flow` の CSS specificity 変更を撤回した。#125 の
  連絡先追加注意書きが無傷であることを diff で確認した。`scripts/landing-page-no-backup-claim.test.ts`
  を新規に追加し、両 LP が JSON Backup を謳わないこと・現状説明を含むこと・#125 の注意書きを
  維持することを固定した。
- 2026-07-23: general-purpose agent で DistributionCapability サブシステムの production 消費者を
  再調査させ、5 ファイルすべてが test-only 参照（`distribution-capability.test.ts` の直接 import と
  `default-agent-model-provider.test.ts` のソーステキスト検査）だけであることを確認した
  （`PassportApp.tsx` / `App.tsx` / `native-agent-model-provider-composition.ts` に実配線なし）。
  `git rm` で 6 ファイル（`default-distribution-capability.ts` / `.native.ts` / `.web.ts`、
  `distribution-capability.ts`、`distribution-capability-notice.ts`、
  `distribution-capability.test.ts`）を削除し、`default-agent-model-provider.test.ts` の
  test-only 参照（Issue 28 の Runtime capability 検査、App Composition 契約テストとは別）を
  削除に追従させた。未使用になった `settings.distribution` i18n メッセージ（型・ja・en）を
  `messages.ts` から除去した。ADR-0033 と `docs/design/distribution-tiers.md` を「削除完遂」の
  内容へ更新した。`bun run typecheck` と関連テストが green であることを確認した。
- 2026-07-23: `diagnostic-report.ts` の `ERROR_PHASES` から producer 不在の `backup-import` を、
  `DiagnosticStorageSummary` から常に `0` の `backupCacheCount` を除去した
  （`backup-export` は `use-local-diagnostics-flow.ts` に現役の producer があるため維持）。
  `local-data-control.ts` の `LocalDataPreview` / `previewFrom`、`use-local-diagnostics-flow.ts` の
  `reportStorage`、`LocalDiagnosticsScreen.tsx` の合計計算からも一貫して除去した。共有される
  診断 JSON の形が変わるため `DIAGNOSTIC_REPORT_SCHEMA_VERSION` を 1 → 2 へ上げ、理由をコード
  コメントで明示した。対応するテスト（`diagnostic-report.test.ts` / `local-data-control.test.ts` /
  snapshot）を更新し、schema v1 → v2 の破壊的変更を固定する新規テストを追加した。
  `docs/design/local-diagnostics-and-erasure.md` の allowlist 表と本文も同期した。
- 2026-07-23: `AppScreen.tsx` の `brandRow` に `flexWrap: 'wrap'`、`brandLockup` に
  `flexShrink: 1` / `minWidth: 0`、`brand` Text に `flexShrink: 1` を追加し、200% Text・
  狭幅端末でヘッダー（BrandMark ロックアップと JA/EN トグル）がクリップ・重なりにならない
  ようにした（通常サイズでは見た目が変わらないことを既存の `app-screen.test.ts` で確認済み）。
  `font-scaling.test.ts` の走査対象に `AppScreen.tsx` を追加し、上記 3 スタイルの存在を
  固定する新規テストを追加した。
- 2026-07-23: このセッションの worktree に `node_modules` が存在しないことに気付き
  （`bun test` / `tsc` は親ディレクトリの `node_modules` へ解決が上に抜けるため気付かず
  動いていたが、`node_modules/.bin/jscpd` は明示パスのため見つからなかった）、
  `make install` で解消した。新規テストファイル 1 件が Biome のフォーマットに合わず
  `bunx biome check --write` で自動整形した。`make before-commit` を再実行し exit 0（1250 pass、
  100% coverage）を確認した。`make release_candidate` は上記の pre-existing `.eas` gap で
  引き続き失敗すること（follow-up 記録済み、本 PR の scope 外）を再確認した。`make dead_code` の
  `InteractionDiscoveryProvider` 未使用型は変更前後で同一（`git stash` で比較）であり、
  本セッションの変更による新規 dead code ではないことを確認した。

### [クラウド基礎クイズと進捗スタンプ] - 2026-07-23

#### 目的

Issue 110（正本）。クラウド基礎の四択クイズ 16 問をアプリへ同梱し、端末内で採点・保存する。
合格状況を「ファミコン風の 16 進数ビットマスク」として既存の自己紹介カード QR に相乗りさせ、
TenkaCloud ブランドの文脈づけと勉強会での会話のフックを作る。実装は Sonnet、PR 作成まで
（マージは呼び出し元 Fable が Codex レビュー後に行う）。

#### 制約

- Passport の哲学（サーバーなし・データは端末内のみ・受け手はアプリ不要）を崩さない。
- クイズ内容はアプリ同梱のみ（TenkaCloudChallenge 等本体への通信・依存を持たない）。
- `rm` / `npx` 禁止（`git rm` / `bunx` / `nlx`）。git add は明示ファイルのみ。
- 8081 kill 禁止、`ios/` / `node_modules` / `site/c/` は不触の原則を保つが、本 Issue は
  `site/c/index.html`（ビューア）自体の変更を明示的に要求しているため、その 1 ファイルに限り編集する。
- 設問内容は事実正確性を最優先する（誤った技術情報を出さない）。
...
3. Storage: `quiz-progress-storage.ts` Port + Web/Native adapter + factory（`intro-card-storage.ts`
   と同型）。
4. Protocol: `intro-card-url.ts` へ任意キー `q` を追加、`encodeIntroCardUrlBestEffort` で QR byte
   予算超過時に `q` を優先度低く落とす。
5. Viewer: `site/c/index.html` にマス目グリッド + "N / 16 クリア" 表示を追加し、
   `scripts/intro-card-viewer.test.ts` で drift を固定する。
6. UI: `QuizScreen.tsx`（一覧・出題・採点・解説）、`i18n/messages.ts` に `quiz` セクション追加、
   `PassportApp.tsx` / `SettingsScreen.tsx` へ Settings 経由の導線を追加。
7. `docs/privacy/data-inventory.md` にクイズ進捗の行を追加。
8. テスト: TDD・BDD 日本語・No Mock・カバレッジ 100%。

#### 検証手順

- `bun test`（カバレッジ 100%）、`bun run typecheck`、`bunx biome check`。
- `bun scripts/architecture-harness.ts --staged --fail-on=error`。
- `make before-commit` が exit 0。
- code-reviewer レビュー（設問の事実正確性・QR byte 予算・allowlist 二重更新漏れ・後方互換・
  アクセシビリティ契約を重点確認）。
...
- 2026-07-23: Domain（`quiz-catalog.ts` 16 問・`quiz-progress.ts`・`quiz-progress-code.ts`
  ...
  超過時に `q` を優先度低く落とす）、Viewer（`site/c/index.html` のマス目グリッド表示、
  `scripts/intro-card-viewer.test.ts` の drift 固定）、UI（`QuizScreen.tsx`、`messages.ts` の
  `quiz` セクション、`PassportApp.tsx` / `SettingsScreen.tsx` の Settings 経由導線）を TDD で
  実装した。`make before-commit` が exit 0（1330 pass、カバレッジ 100%）になることを確認した。
- 2026-07-23: code-reviewer レビューを実施。結論は「ブロッカーなし、16 問の事実正確性は
  問題なし」。指摘を反映した: (1) major — `docs/privacy/data-inventory.md` の削除契機列が
  実装にない「個別削除」を謳っていたため「アプリ削除のみ（個別削除 UI は scope 外、
  follow-up）」へ訂正、(2) minor — 未使用だった `QUIZ_CATALOG_VERSION` を
  `serializeQuizProgress` の保存 JSON へ実際に組み込み、(3) minor — `QuizScreen.tsx` が
  `clearedIds.size` を直書きし `quizClearedCount`/`isQuizComplete` を使っていなかった重複を
  解消し、全問クリア時の `allClearedNotice` 表示を追加、(4) nit — `handleQuizQuestionCorrect`
  が `setState` の関数型 updater 内で `quizProgressStorage.save` を呼ぶ副作用パターンを、
  `introCardDraftHydrated` と同じ「Hydrated フラグ + 専用 `useEffect`」の流儀へ分離した。
  クイズ進捗を Diagnostics の「全データ削除」対象に含めるかどうかは owner 判断が必要な
  scope 外の論点のため `/follow-up add` で記録した（severity: medium）。反映後、
  `make before-commit` を再実行し exit 0（コミット前の最終確認は本セクション末尾を参照）。

#### 振り返り

- **問題**: `SettingsScreen.tsx` に「クイズに挑戦」ボタンを 1 つ足しただけで Cognitive
  Complexity が上限（15）を 16 に超過した。
  **根本原因**: Local Model 管理セクション（複数の条件付き表示を含む JSX）が
  `SettingsScreen` 本体に直書きされたまま肥大化しており、複雑度の余裕がほぼ無い状態だった。
  **予防策**: 導線ボタンを 1 つ足す変更が来ることを見越し、複雑度に余裕のない画面へ機能を
  足す前に必ず `bunx biome check` で複雑度を確認する。今回は `LocalModelCard` /
  `LocalModelCandidateCard` と同じ「子 Component へ切り出す」既存方針を
  `ModelManagementSection` として素直に適用し解消した（新しい方針の発明ではなく、
  既存パターンの追従で足りた）。
- **問題**: `q`（進捗ビットマスク）を fragment のスキーマ検証に混ぜる際、
  `decodePayload`（`IntroCard` を返す）の内部で発生する `SchemaValidationError` を
  `INVALID_SHARE_URL` へ変換する `try/catch` を、リファクタリング時に誤って
  `parseIntroCardUrlFragmentPayload` の外へ出してしまい、既存テスト
  （links が配列でない場合の拒否）を一時的に壊した。
  **根本原因**: 「JSON parse + schema 検証」と「`createIntroCard` へのフィールド変換」の
  2 段階のうち、後段の error 変換だけを見落とした。
  **予防策**: 共通の `asShareUrlResult` ヘルパーへ抽出し、両段階が同じ変換ロジックを
...
### [Issue 138 実機 blocker 3 件（Settings デバッグ化・DL 完了フリーズ・会話 Agent クラッシュ）] - 2026-07-24

#### 目的

owner が TestFlight 実機で確認した公開 blocker 3 件を修正する。A: Qwen ダウンロード
100% 到達後に全ボタンが灰色のまま固まる。B: Settings が開発者向けデバッグメニュー化
している。C: 会話 Agent 起動でクラッシュする（原因未確定）。

#### 制約

- `rm` / `npx` 禁止（`bunx` / `nlx`）。8081 kill 禁止。ios/ node_modules 不触。
- モックデータ・スタブ API 禁止。型エスケープ禁止。
- 消費者に開発者機能を見せない。「Development Build だけ」等の文言を消費者導線に
  残さない。
- Bonsai は既定 off、production entitlement は付けない。
- `make before-commit` 100% カバレッジを維持する。

#### タスク

- A: `SettingsScreen.tsx` の過剰 disable（quiz / 会話 Agent / 戻る が
  `modelManagement?.busy` に連動していた）を是正する。`createLocalModelOperationLane`
  の busy 完了/失敗遷移は既存実装で正しく（`.finally()` で必ず `onFinish` が発火）
  実装済みと判明したため、production code の変更は over-disable の是正のみとし、
  `trusted-model-enablement-controller.test.ts` に Operation Lane 経由の実行テスト
  （成功・INTEGRITY_MISMATCH・RESOURCE_BLOCKED の 3 パターンで busy が
  true→false へ必ず戻ることを実行で固定）を追加した。
- B: 当初は `__DEV__` ゲートで開発者向け UI を隠す予定だったが、コーディネータ
  からの途中方針変更（dev ビルドでは変化が確認できないため）を受けて、生の GGUF
  選択・`LocalModelCard` 一覧・import candidate カード・診断ボタン・Pilot
  Measurement ボタンを **全ビルドから完全に除去**する方針へ切り替えた。
  `OnDeviceAiSection`（Qwen 有効化。既に消費者向け文言）・`cautionAssessment`
  確認カード・`pendingProviderOperation` 確認カード（Qwen フローと共有する機構）は
  維持する。消費者向けの簡潔な「全データ削除」導線を新設し、既存
  `useLocalDiagnosticsFlow` の erasure 経路（`requestDeleteAll` /
  `confirmDeleteAll` / `cancelDeleteAll` / `recoveryRequired` の再試行）を
  そのまま再利用する（新しい削除ロジックは作らない）。
- C: `ErrorBoundary`（class component、`src/components/ErrorBoundary.tsx`）を新設し、
  `PassportApp.tsx` の `conversation-agent` stage をこれで包む。既存コードを
  監査した結果、`peers[0]` アクセス・i18n `conversationAgent` の全 key・
  `provider`/`providerRunner`（型上 null になり得ない）・`providerRunner.run` の
  失敗（`resolveConversationAgentRun` が既に try/catch 済み）はいずれも
  問題なし。JS 例外に対する最後の防波堤として ErrorBoundary を追加した。
  Native 側の crash は JS の Error Boundary では捕まらないため、owner から
  クラッシュスタックが届いたら別途 precise fix を追記する。

#### 検証手順

- `make before-commit`（architecture-harness、harness_test、release_test_coverage、
  pre_release_check、dup_check、lint_text、lint、typecheck、app_test、web_export）
  が exit 0 になることを確認する。
- owner のシミュレーター / 実機で、Settings が clean になったこと（診断・Pilot・
  生 GGUF 選択が一切表示されない）、Qwen ダウンロード完了後も画面操作ができること、
  会話 Agent を開いてもアプリ全体がクラッシュしないことを確認する（このセッションの
  環境では iOS ビルドが完走しないため、実機確認は owner に委ねる）。

#### 進捗ログ

- `SettingsScreen.tsx`: quiz / 会話 Agent（`!hasIntroCard` のみへ変更）/ 戻る の
  busy disable を除去。診断・Pilot Measurement ボタン、生の GGUF 選択・
  `LocalModelCard` 一覧・import candidate カードを完全に除去。消費者向け
  `DataErasureSection`（`SettingsDataErasureProps`）を新設。
- `PassportApp.tsx`: `<SettingsScreen>` から `onOpenDiagnostics` /
  `onOpenPilotMeasurement` を削除し、代わりに `dataErasure` prop
  （`diagnosticsFlow` の erasure 経路を再利用）を配線。`conversation-agent`
  stage を `ErrorBoundary` で包む。
- `src/components/ErrorBoundary.tsx` を新設（`error-boundary.test.ts` で
  source-text 検査）。
- `src/app/i18n/messages.ts`: `settings.eraseAllData*`（7 key）、
  `conversationAgent.crashNotice*`（2 key）を ja / en / 型定義に追加。
- テスト: `settings-accessibility.test.ts` を全面書き換え（開発者向け UI 不在の
  固定、over-disable 是正の固定、DataErasureSection の固定）。
  `pilot-measurement-accessibility.test.ts` / `local-diagnostics-wiring.test.ts`
  の Settings 導線前提のテストを新方針（除去済みであることの固定）へ更新。
  `trusted-model-enablement-controller.test.ts` に busy 完了/失敗遷移の実行
  テストを追加。
- `make before-commit` が exit 0（1958 テスト、100% カバレッジ、dup_check OK、
  web export 成功）であることを確認。
- フォローアップ 4 件を記録（開発者向け受け皿の再設計、RN 描画テスト基盤の導入、
  native hang への timeout セーフティネット、未使用 i18n key の削除）。

#### 振り返り

- **問題**: 当初 B の対応方針（`__DEV__` ゲート）で `SettingsScreen.tsx` を実装
  済みだったが、コーディネータから「dev ビルドでは変化が確認できない」という
  指摘で全ビルド除去へ方針転換となり、実装をやり直した。
- **根本原因**: `__DEV__` ゲートは「開発者に見せたまま消費者にだけ隠す」という
  要件を満たすが、owner が確認手段として使う想定の環境（シミュレーター /
  dev ビルド）では効果が見えないという、確認手段と実装方針の不一致を最初の
  設計段階で詰め切れていなかった。
...
### 会話エージェントを checkbox 一致から引用による共通点提示へ（Issue 147） - 2026-07-25

#### 目的

「ローカル LLM をダウンロードしてエージェントが共通点を探す」と提示している機能が、
実際にはカタログ checkbox の共通集合でしかなく、モデルが 3 層すべてで排除されていた
問題を直す。入力層は自由記述を捨て、契約層はモデルの出力を Rules の結論の部分集合に
制限し、合成層は ADR-0038 が Provider を Rules 固定にしていた。

#### 制約

- 目の前にいる実在の人物について、事実でない共通点を表示しない。
- モデルを持たない端末・Expo Go・Web の挙動を変えない。
- 端末外への送信を増やさない。
- カバレッジ 100％を維持する。

#### タスク

1. `grounded-quote-bridge.ts` を新設し、引用が入力文の部分文字列であることの照合と、
   長さ・制御文字・連絡先混入の拒否を純関数として実装する。
2. `AgentModelInput` に自由記述（両者分揃ったときだけ）を追加する。
3. 出力契約に `grounded-bridge` を追加し、Validator が引用を照合してから固定
   テンプレートへ埋める。
4. Safety Boundary が自由記述を payload へ載せ、揃ったときだけ Schema へ
   grounded-bridge を加える。
5. ADR-0038 の Provider 固定を会話エージェントに限って戻す。
6. ADR-0043 に判断を記録する。

#### 検証手順

- `bun test src --coverage`: 1523 pass、100％維持。
- `bun run typecheck` / `bun scripts/architecture-harness.ts` / `check-duplication`。
- Owner による実機確認（Model を持つ Development Build で、checkbox が一致しない
  ペアでも自己紹介文から共通点が出ること）。`llama.rn` の実行経路は `bun test` で
  検証できないため本 Repository では完結しない。

#### 進捗ログ

- 2026-07-25: owner から「ローカルモデルなら Privacy は問題ないのでは」と指摘を受け、
  自分の整理が誤っていたことを確認した。端末内で完結する限り新しく外へ出るものは無く、
  実際の障害は Privacy ではなく ADR-0038 の実機不具合と、出力の正確さだった。
- 2026-07-25: モデルに自由記述を書かせる案を捨て、「根拠になった箇所をそのまま
  抜き出させ、入力文の部分文字列であることを照合する」形を採った。幻覚を後段で
  弾くのではなく構造的に不可能にできる。
- 2026-07-25: `grounded-quote-bridge.ts` を 14 件のテストで固定し、契約・Safety
  Boundary・合成層を通して `bun test src --coverage` 1523 pass・100％を確認した。

#### 振り返り

- **問題**: 「エージェントが共通点を探す」という説明と実装の乖離が、3 層に分かれて
  いたため、どれか 1 つを直しても解消しない状態になっていた。
- **根本原因**: Issue 104 の受入基準「自己紹介自由文を渡さない」が、Provider が
  端末内に閉じているという前提と結び付かないまま、契約として固定されていた。
  保守的な線引きそのものは正しかったが、前提が変わった後も見直されなかった。
