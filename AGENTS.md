...
bunx expo install --fix -- --ignore-scripts # lifecycle script を止めて Expo 互換バージョンへ整合
make dev          # 開発サーバー起動（Development Build / dev-client 向け。Expo Go は make start）
```

Metro/Expo が残ってポートが解放されない場合は `make stop` で Expo/Metro プロセスだけを
安全に停止できる。`make`（引数なし）または `make help` で使い分けを確認できる。

Expo アプリはリポジトリルートに置く。単一アプリの間は Hono / Vite workspace を作らない。
Expo Go / Web は Rules Provider を使い、Native module は Development Build 専用境界へ隔離する。
...
| --- | --- |
| `ni` | 依存関係インストール |
| `nr dev` / `make dev` | Development Build (dev-client) 向けに開発サーバー起動。Expo Go には接続不可 |
| `make start` | スマホ (Expo Go 向け、`--go`) に QR 付きで起動。dev-client 版とは別 Build |
| `make stop` | 8081 で待ち受ける Expo/Metro だけを安全に停止 (無関係なプロセスは kill しない) |
| `make restart` | `make stop` の後に `make dev` で再起動 |
| `make help` (引数なしの `make` でも同じ) | start/dev/stop の使い分けを含む主要 target 一覧を表示 |
| `nr test` | テスト実行 |
| `nr typecheck` | TypeScript 型チェック |
| `nr build:web` | Web Export |
| `nr ios` | iOS で Expo を起動 |
| `nr android` | Android で Expo を起動 |
| `nr web` | Web で Expo を起動 |
| `make before-commit` | 品質ゲート一括 |
| `make harness_test` | harness 検出ロジックのテスト |
| `make dup_check` | jscpd baseline ratchet (重複が baseline を超えたら fail、ADR-0012) |
| `make dup_baseline` | 重複 baseline を現状で更新 (増加は PR body で理由を説明) |
| `make dup_report` | jscpd の全クローンを表示 |
| `make dead_code` | knip 未使用コード検出 (報告のみ、fail しない) |
