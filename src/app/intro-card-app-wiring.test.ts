...
describe('自己紹介カードピボット Step 1（Issue 79）のメインフロー配線契約', () => {
  ...
  it('起動時の Intro Card Notice は effective locale が確定した後にだけ組み立てる（Codex Finding 1: 保存済み選好の反映前に locale 依存の起動通知を作らない）', async () => {
    ...
    const thenStart = effectBody.indexOf(']).then(');
  ...
  it('ProfileHomeGate から IntroCardStageGate へ言語切替・編集・削除・Settings 導線を渡す（Issue 130: #127 が外した Settings 導線をクイズ・診断への唯一の入口として復活）', async () => {
    ...
    expect(text).toContain('onOpenSettings={openSettings}');

    const introCardEditStart = text.indexOf('introCardEdit={{');
    const introCardEditEnd = text.indexOf('\n      }}', introCardEditStart);
    const introCardEditBody = text.slice(introCardEditStart, introCardEditEnd);
    expect(introCardEditBody).toContain('onOpenSettings: openSettings,');
  });

  it('IntroCardStageGate は IntroCardScreen・IntroCardEditScreen の両方へ言語切替・Settings 導線をそのまま渡す（Issue 130）', async () => {
    const text = await source();
    ...
    expect(editBlock).toContain('onOpenSettings={edit.onOpenSettings}');
  });
